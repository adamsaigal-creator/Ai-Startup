...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getPage("neighbourhoods-milton");
  return buildPageMetadata({
    title: page?.seo_title ?? "Milton Real Estate — Saigal Realty Inc., Brokerage",
    description: page?.seo_description ?? "Milton real estate — homes for sale, market stats, schools, parks, and commute.",
    path: "/neighbourhoods/milton",
...
export default async function Page() {
  const [page, settings, listings] = await Promise.all([
    getPage("neighbourhoods-milton"),
    getSiteSettings(),
    getFeaturedListings("Milton", 3),
  ]);
    ...
    <>
      ...
      <div style={{ fontFamily: "var(--font-work-sans), sans-serif", color: "oklch(23% 0.012 60)", background: "oklch(97% 0.012 75)", width: "100%", overflowX: "hidden" }}>
        ...
        <section style={{ position: "relative", height: "56vh", minHeight: "420px", width: "100%", marginTop: "18px" }}>
          <img alt="Milton streetscape / downtown" id="milton-hero" src={c.hero.image} style={{ objectFit: "cover", position: "absolute", inset: "0", width: "100%", height: "100%" }} />
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "28px" }}>
              {listings.map((listing, i) => (
                <div key={listing.id} style={{ background: "oklch(99% 0.004 90)", borderRadius: "4px", overflow: "hidden" }}>
                  <img loading="lazy" alt="Listing photo" id={`listing-${i + 1}`} src={listing.image ?? "/images/neighbourhoods-hero.png"} style={{ objectFit: "cover", width: "100%", height: "220px" }} />
