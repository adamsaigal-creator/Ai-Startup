...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildPageMetadata({
    title: page?.seo_title ?? "Sell Your Home — Saigal Realty Inc., Brokerage",
    description: page?.seo_description ?? "A precise market analysis and a marketing plan built for today's buyer.",
    path: "/sell",
...
export default async function Page() {
  const [page, settings] = await Promise.all([getPage("sell"), getSiteSettings()]);
  const c = page!.content;

  return (
    <>
      <SiteHeader nav={NAV} activeLabel={"Sell"} ctaLabel="Book a Consultation" ctaHref="/contact" ctaSize="sm" logoUrl={settings.logo_url ?? undefined} brokerageName={settings.brokerage_name} />
      <div style={{ fontFamily: "var(--font-work-sans), sans-serif", color: "oklch(23% 0.012 60)", background: "oklch(97% 0.012 75)", width: "100%", overflowX: "hidden" }}>
        <section style={{ position: "relative", height: "52vh", minHeight: "420px", width: "100%" }}>
          ...
          <img alt="Beautifully staged home exterior" id="sell-hero" src={c.hero.image} style={{ objectFit: "cover", position: "absolute", inset: "0", width: "100%", height: "100%" }} />
        ...
        <section style={{ padding: "56px 56px 20px", maxWidth: "820px", margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "var(--font-cormorant-garamond), serif", fontSize: "26px", fontWeight: "600", margin: "0 0 14px" }}>{c.valuationCta.headline}</h2>
          <p style={{ fontSize: "15px", color: "oklch(46% 0.02 60)", margin: "0 0 24px" }}>{c.valuationCta.body}</p>
          <a href={c.valuationCta.buttonHref} style={{ display: "inline-block", padding: "15px 36px", background: "oklch(58% 0.16 45)", color: "oklch(99% 0.004 90)", borderRadius: "2px", fontSize: "14px", fontWeight: "600", letterSpacing: "0.03em" }}>{c.valuationCta.buttonLabel}</a>
        </section>
        <section style={{ padding: "100px 56px 40px", maxWidth: "1200px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "56px" }}>
            <span style={{ fontSize: "13px", letterSpacing: "0.24em", textTransform: "uppercase", color: "oklch(58% 0.16 45)" }}>{c.steps.eyebrow}</span>
            <h2 style={{ fontFamily: "var(--font-cormorant-garamond), serif", fontSize: "34px", fontWeight: "600", margin: "14px 0 0" }}>{c.steps.headline}</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: "32px" }}>
            {c.steps.items.map((step: { number: string; title: string; body: string }) => (
              <div key={step.number}>
                <span style={{ fontFamily: "var(--font-cormorant-garamond), serif", fontSize: "28px", color: "oklch(58% 0.16 45)" }}>{step.number}</span>
                <h3 style={{ fontFamily: "var(--font-cormorant-garamond), serif", fontSize: "19px", fontWeight: "600", margin: "12px 0 8px" }}>{step.title}</h3>
                <p style={{ fontSize: "14px", lineHeight: "1.65", color: "oklch(46% 0.02 60)" }}>{step.body}</p>
              </div>
            ))}
          </div>
        </section>
        <section style={{ padding: "0 56px 100px", maxWidth: "1000px", margin: "0 auto", textAlign: "center" }}>
          <span style={{ fontSize: "13px", letterSpacing: "0.24em", textTransform: "uppercase", color: "oklch(58% 0.16 45)" }}>{c.difference.eyebrow}</span>
          <h2 style={{ fontFamily: "var(--font-cormorant-garamond), serif", fontSize: "30px", fontWeight: "600", margin: "16px 0 40px" }}>{c.difference.headline}</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: "32px" }}>
            {c.difference.stats.map((s: { value: string; label: string }) => (
              <div key={s.value} style={{ padding: "32px 24px", background: "oklch(99% 0.004 90)", borderRadius: "4px" }}>
                <span style={{ fontFamily: "var(--font-cormorant-garamond), serif", fontSize: "32px", color: "oklch(58% 0.16 45)" }}>{s.value}</span>
                <p style={{ fontSize: "13px", lineHeight: "1.6", color: "oklch(46% 0.02 60)", margin: "10px 0 0" }}>{s.label}</p>
              </div>
            ))}
          </div>
        </section>
